...
class Store {
  ...
  void triggerOnAction<T>(FluxAction<T> action, [dynamic Function(T payload)? onAction,]) {
    if (onAction != null) {
  ...
  void triggerOnConditionalAction<T>(
      FluxAction<T> action, FutureOr<bool> Function(T payload) onAction,) {
    assert(action != null);
    action.listen((dynamic payload) async {
      ...
      bool wasChanged;
      wasChanged = result is Future ? await result as bool : result as bool;
      if (wasChanged) {
        trigger();
      }
    });
  ...
  StreamSubscription<Store> listen(void Function(Store event) onData,
